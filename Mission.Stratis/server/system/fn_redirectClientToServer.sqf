/*
	...
	Description:
	Transfer a client from this server to another dedicated server.
	PARAMS:
		0 (STRING):
			IP direction of the target server.
		1 (STRING):
			PORT of the target server.
		2 (STRING):
			PASSWORD of the target server (if any).
		3 (NUMBER):
			Timeout before dropping redirect.
